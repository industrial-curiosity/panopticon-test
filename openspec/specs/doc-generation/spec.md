...
then produce constrained YAML frontmatter with a non-empty `type` field for
each non-reserved generated concept document, generate required progressive-
disclosure indexes, and preserve deterministic interface-doc content. When the
feature is disabled, existing four-layer generation requirements SHALL continue
without requiring OKF feature artifacts or validation.
