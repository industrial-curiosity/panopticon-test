...
| `tests/test_drift.py` | Doc-drift verdict parsing, loud failures on malformed verdicts, PR-comment/step-summary report formatting including per-doc-type remediation (`interfaces.md`'s specific `python3 -m panopticon.docs render` command vs. the panopticon-doc-generation skill for the other three layers) and the same-branch-push/auto-rerun statement |
