...
| `tests/test_drift.py` | Doc-drift path classification and planner validation, metadata-only clean verdicts, isolated batch evaluator prompts, aggregate clean/stale results, safe stage-aware timeout recovery, verdict parsing, report formatting, action collection, and the CLI exit-code contract. |
